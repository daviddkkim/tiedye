...
const SpaceDropdown = () => {
    ...
    const getCurrentSpaceName = () => {
        const matchSpace = spaces?.filter((space) => { return space?._id.toString() === spaceId })[0];
            ...
            <DropdownMenu.RadioGroup>
                {spaces && spaces.map((space) => {
                    if (space) {
                        ...
                        return (
                            <DropdownMenu.RadioItem onClick={() => { handleItemClick(space) }} value={spaceId} key={spaceId}>
                                {space?.name}
